...
<div class="card">
	<div class="card-header">
		<strong><?= h($category->name). ' - '.$year ?></strong> Archived Answers
	</div>
	...
	<div class="form-group row">
		<div class="col-sm-12"><label class="form-control-label"><?= h($clientQuestion->question->question) ?></label></div>
		<?= $this->element('Question_Type/'.$clientQuestion->question->question_type->name, ["key" => $key,"question" => $clientQuestion->question]) ?>
	</div>
	<hr/>
	<?php endforeach; 
	  $categories = $this->Category->getCategories($activeUser['contractor_id'], $service_id,1); 	
