...
$uploaded_path = Configure::read('uploaded_path');
    ...
    foreach($question['client_questions'] as $k => $v) {
       ...
       <div class="col-sm-9 uploadWraper">
            <?php echo $this->Form->control('contractor_answers.'.$key.'.'.$k.'.answer', ['value'=>$val, 'label'=>false, 'type'=>'hidden', 'class'=>'documentName']); ?>
            <?php echo $this->Form->file('contractor_answers.'.$key.'.'.$k.'.uploadFile', ['label'=>false, 'accept' => '.images/*, .pdf,.xls, .xlsx', 'style'=> $fileCss]); ?>
            ...
            <?= $this->Form->control('contractor_answers.'.$key.'.'.$k.'.filenmHandle', ['type'=>'hidden', 'value'=>$filenmHandle, 'class'=>'filenmHandle']); ?>
...
<?php }
    ...
    $filenmArr[] = $contractor_id;
    $filenmArr[] = $question->id;
    $filenmHandle = implode('-', $filenmArr);$fileCss = $val!='' ? "display:none;" : "";
    ?>
    <div class="col-sm-12 uploadWraper">
        <?php echo $this->Form->control('contractor_answers.'.$key.'.answer', ['value'=>$val, 'label'=>false, 'type'=>'hidden', 'class'=>'documentName']); ?>
        <?php echo $this->Form->file('contractor_answers.'.$key.'.uploadFile', ['label'=>false, 'accept' => '.images/*, .pdf,.xls, .xlsx', 'style'=> $fileCss]); ?>
        <div class="uploadResponse">
        <?php if($val!='') { ?>
        <a href="<?= $uploaded_path.$val ?>" class="uploadUrl" data-file="<?= $val ?>" target="_Blank"><?= $val ?></a>
        <?= $this->Html->link('<i class="fa fa-times-circle"></i>', ['controller'=>'Uploads', 'action' => 'deleteFile', $val], ['escape'=>false, 'title' => 'Remove', 'class'=>'ajaxfileDelete btn btn-sm']);?>
        ...
        <?= $this->Form->control('contractor_answers.'.$key.'.filenmHandle', ['type'=>'hidden', 'value'=>$filenmHandle, 'class'=>'filenmHandle']); ?>
