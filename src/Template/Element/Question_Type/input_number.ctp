...
//$val= !empty($question->contractor_answers) ? $question->contractor_answers[0]->answer : '';
?>
<?php
if($question['client_based']) {
	...
	<div class="col-sm-9 mb-2">
  		<?= $this->Form->control('contractor_answers.'.$key.'.'.$k.'.answer',['type'=>'number', 'class'=>'form-control '.$question->safety_type, 'label'=>false, 'value'=>$val]) ?>	      
...
?>
	...
	<?= $this->Form->control('contractor_answers.'.$key.'.answer',['type'=>'number', 'class'=>'form-control '.$question->safety_type, 'label'=>false, 'value'=>$val]) ?>
