...
if($question['client_based']) {
	foreach($question['client_employee_questions'] as $k => $v) { 
        ...
        $val='';
        $val1 ='';
        if(!empty($question->employee_answers))
        {
...
?>
	...
	<div class="col-sm-9 mb-2">
		<div class="row">
        <?= $this->Form->select('employee_answers.'.$key.'.'.$k.'.answer', $question_options, ['class'=>"col-sm-4 form-control other_textfield", 'empty'=>true, 'multiple'=>$multiselect, 'default'=>$val]) ?>
        <div class="answer_others col-sm-6" style=<?= $css ?>>
        <?= $this->Form->control('employee_answers.'.$key.'.'.$k.'.answer_other', ['class'=>"form-control", 'empty'=>true, 'value'=>$val1, 'label'=>false]) ?>
        </div>
        </div>
...
if(!empty($question->employee_answers))
{
    if ( strstr( $question->employee_answers[0]->answer, 'other:' ) ) {
    $valother = explode(':', $question->employee_answers[0]->answer);
    $val = $valother[0];
    $val1 = $valother[1];
    } else {
    $val=explode(',',$question->employee_answers[0]->answer);
    }
}
$css='display:none';
if($val1!='') { $css="display:block"; }
...
<div class="row">
<?= $this->Form->select('employee_answers.'.$key.'.answer', $question_options, ['class'=>"col-sm-4 form-control other_textfield", 'empty'=>true, 'multiple'=>$multiselect, 'default'=>$val]) ?>
<div class="answer_others col-sm-6" style=<?= $css ?>>
<?= $this->Form->control('employee_answers.'.$key.'.answer_other', ['class'=>"form-control", 'empty'=>true, 'value'=>$val1, 'label'=>false]) ?>
</div>
</div>
