...
<div class="card">
	...
	<tr>		
		...
		<td>
		<?php if(in_array($employee['id'], $contractor_emp))  {
			   echo '<span class="">Already associated</span>';
		}else 
		{
		  if($employee['requeststatus'] == "")  {
		  	if($available==0) {
		  		echo $this->Html->link(__('Send Request'), ['controller'=>'ContractorSites', 'action'=>'add-slot'], ['escape'=>false, 'title' => 'Send Request']);
		  	}else{
			   echo $this->Html->link(__('Send Request'), ['controller'=>'ContractorRequests', 'action'=>'add',$employee['id']], ['class'=>'btn-link ajaxmodal', 'data-toggle'=>'modal', 'data-target'=>'#scrollmodal']);
		  	}
			} 
			else {
